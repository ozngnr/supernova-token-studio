...
export const groupByKeyAndName = (
  tokenArray: OriginalFormatTokenInterface[] | StandardTokenInterface[],
  ...
  const groupedTokens = tokenArray.map((token) => {
    ...
    const groupsFromName = token.name
      .split('/')
      .map((group) => transformName(group, userSettings.nameConversion))

    // console.log(groupsFromName)
  ...
  return groupedTokens.reduce((accumulator = {}, currentValue) =>
    deepMerge(accumulator, currentValue)
  )
}
